...
uint256 hashGenesisBlock("0x00000f393b457880dfdd06c9c81024c8813e000db7fbd1a81c66ff90daa85259");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MMTCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MMTCoin Signed Message:\n";
...
{
    // MMTCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 3 * 60; // MMTCoin: each block
static const int64 nTargetSpacing = 3 * 60; // MMTCoin: 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MMTCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, MMTCoin - DarkGravity, written by Evan Duffield - evan@MMTCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MMTCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MMTCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        block.nVersion = 1;
        block.nTime    = 1501618637;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3624398;
        ...
        assert(block.hashMerkleRoot == uint256("0x61911f1f1500a17441e957253fab8a5b29c6865d4a0deddc53a41de0bc9cd2ad"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // MMTCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MMTCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MMTCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MMTCoinMiner(CWallet *pwallet)
{
    printf("MMTCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MMTCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running MMTCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MMTCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MMTCoinMiner, pwallet));
}
